...
async function main() {
  ...
  if (!rawKey) {
    console.error('Usage: node disable-license.js --license TAR-XXXX-XXXX-XXXX');
    process.exit(1);
  }
  ...
  try {
    ...
    if (license.status === 'disabled') {
      console.log('License is already disabled.');
      process.exit(0);
    }
    ...
    console.log(`\nSuccessfully disabled license ${license.displayLabel || '(no label)'}.`);
